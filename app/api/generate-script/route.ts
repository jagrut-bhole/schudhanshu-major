...
interface ScriptResponseSchema {
    success: boolean;
    message: string;
    data?: {
        script: string;
...
export async function POST(
    req: NextRequest
): Promise<NextResponse<ScriptResponseSchema>> {
    try {
...
Topic: ${title}
Context: ${description || "A currently trending topic."}
...
Keep the tone: conversational, engaging, easy to understand. Avoid jargon. Write as if speaking directly to a viewer.`;
        ...
        if (!groqResponse.ok) {
            ...
            return NextResponse.json(
                {
                    success: false,
                    message: "Failed to generate script via Groq",
                },
                { status: 502 }
            );
        }
        ...
        if (!script) {
            return NextResponse.json(
                {
                    success: false,
                    message: "Groq returned an empty script",
                },
                { status: 502 }
            );
        }
